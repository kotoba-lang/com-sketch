(ns sketch.main
  "Kotodama WASM entrypoint for the Sketch clean-room actor (L4) — Clojure port.
  ...
  facts (`sketch.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sketch")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sketch.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sketch-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sketch-compat/ready)
